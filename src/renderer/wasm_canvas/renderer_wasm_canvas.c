...
EM_JS(void, gns_canvas_init, (int w, int h), {
    var canvas = Module['canvas'] || document.getElementById('gns-canvas') || document.querySelector('canvas');
    if (!canvas) {
        canvas = document.createElement('canvas');
        canvas.id = 'gns-canvas';
    ...
    canvas.width = w;
    canvas.height = h;
    canvas.style.setProperty('--gns-aspect', String(w) + ' / ' + String(h));

    var ctx = canvas.getContext('2d', { alpha: false });
    ctx.imageSmoothingEnabled = false;

    Module['gnsCanvas'] = canvas;
    Module['gnsCanvasContext'] = ctx;
    Module['gnsCanvasImageData'] = ctx.createImageData(w, h);
});

EM_JS(void, gns_canvas_present, (const unsigned char *pixels, int w, int h), {
    var ctx = Module['gnsCanvasContext'];
    var imageData = Module['gnsCanvasImageData'];
    if (!ctx || !imageData) return;

    var len = w * h * 4;
    imageData.data.set(Module.HEAPU8.subarray(pixels, pixels + len));
    ctx.putImageData(imageData, 0, 0);
});

EM_JS(double, gns_now_ms, (void), {
    return performance.now();
});
#else
static void gns_canvas_init(int w, int h) { (void)w; (void)h; }
static void gns_canvas_present(const unsigned char *pixels, int w, int h) { (void)pixels; (void)w; (void)h; }
static double gns_now_ms(void) { return (double)clock() * 1000.0 / (double)CLOCKS_PER_SEC; }
#endif
...
static WasmCanvasSurface *load_bmp_rgba(const char *path, int use_colorkey)
{
    ...
    if (!(bpp == 8 || bpp == 24 || bpp == 32)) {
        fprintf(stderr, "WASM Canvas BMP loader only supports 8/24/32-bpp uncompressed BMP: %s\n", path);
        free(file);
        return NULL;
    }
...
int LoadBitmap(char *fname, int bmpindex, int flag)
{
    ...
    if (!surface) {
        fprintf(stderr, "WASM Canvas LoadBitmap failed: %s\n", fname);
        return -1;
    }
